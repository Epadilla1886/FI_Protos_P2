...
app.controller('jsCompCondController', function() {
    ...
    this.features = {
        ...
        'Feature Set 5 - Switch Statement': {
            html: "<ol class=left-border><li>Use the variables from <i>Feature Set 4</i><li>Create a <code>switch</code> statement based on the current day of the week: <code><br>var date = new Date();<br>var dayOfWeek = date.getDay();<br>var message = "";</code><ol class=nested-1><li>7 <code>case</code> statements should check which <code>dayOfWeek</code> it is.<ul><li><b>Set message to:</b> either 'it is the weekend' or 'it is the weekday' depending on what day it is.</ul><li>After the <code>switch</code>, console.log the message.</ol><li>Run your code and view the output in the console.<ul><li>Did you get the results you expected?<li>Try manually setting the dayOfWeek to particular days to test other days. Check out how the Date object works on W3</ul></ol>"
        }
    }
});
