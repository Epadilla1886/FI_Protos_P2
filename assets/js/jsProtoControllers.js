...
app.controller('jsCompCondController', function() {
    ...
    this.features = {
        ...
        'Feature Set 5 - Switch Statement': {
            html: "<ol class='left-border'><li>Create a <code>switch</code> statement based on the current day of the week:<code><br>var date=new Date();<br>var dayOfWeek=date.getDay();<br>var message='';</code><ol class='nested-1'><li>7 <code>case</code> statements should check which <code>dayOfWeek</code> it is.<ul><li><b>Set message to:</b> either 'it is the weekend' or 'it is the weekday' depending on what day it is.</li></ul></li><li>If it is wednesday, add onto the message: 'It is humpday!' if it is Wednesday</li><li>After the <code>switch</code>, console.log the message.</li></ol></li><li>Run your code and view the output in the console.<ul><li>Did you get the results you expected?</li><li>Try manually setting the dayOfWeek to particular days to test other days. Check out how the Date object works on W3</li></ul></li></ol>"
        }
    }
});
...
app.controller('jsLoopsController', function() {
    ...
    this.features = {
        ...
        'Feature Set 1 - More array looping fun!': {
            html: "<ol class='left-border'> <li>Create the <strong>exact</strong> following array <ul> <li><code>var numbers=['zero','one','two','three','four','five','six'];</code> </li></ul> </li><li>Using a <code>for</code> loop, output <strong>all</strong> the numbers in <strong>ascending</strong> order, <code>'zero' through 'six'</code> <ul> <li> <button class='btn btn-danger btn-xs' type='button' data-toggle='collapse' data-target='#hint1' aria-expanded='false' aria-controls='hint1'>View Hint</button> <ul id='hint1' class='collapse'> <li><em>Hint</em> - <code>for(var i=0; i &lt; numbers.length; i++){...}</code> </li><li><em>Hint</em> - Place the <code>console.log()</code> between the <code>{}</code> of your <code>for</code> loop</li><li><em>Reminder</em> - To access values in an array: <code>numbers[0] would give you 'zero'</code> <strong>Try</strong> replacing the <code>0</code> with <code>i</code> </li></ul> </li></ul> </li><li>Save your work and test your code</li><li>Using the same array from above write a different <code>for</code> loop that outputs <strong>all</strong> the numbers in <strong>descending</strong> order, <code>'six' through 'zero'</code> </li><li>Save your work and test your code</li><li>Using the following array: var answers=['no','yes','no','yes','no','yes','no'] write a <code>for</code> loop that only outputs the words 'yes'. You should only modify the starting point of the array and the incrementor of the array</li><li>Write another <code>for loop</code>. Use the mixedArray below <ul> <li><code>var mixedArray=[4, 56, 11, 13, 97, 22, 7, 36, 19, 5, 8, 10];</code> </li><li>Using an if-statement inside, console.log the <em>odd</em> numbers. In this case you should see <code>11, 13, 7, 19, 5</code></li><li>The modulo (%) operator can help you determine if a number is odd or not. Check it out!</li></ul> </li><li>Finally, write the same loop for mixedArray, but now use the array \"push\" method (look it up!) to add all of the odd numbers to a new array. Call this new array <code>oddNumbers</code> <ul> <li><code>push</code>is an array method. It should be used with the oddNumbers array to add a number </li><li>You won't need to change what you are testing in your if statement</li><li>Instead of console logging, push the number into the oddNumbers array</li><li><strong>after</strong> the for loop, use console.log to output the oddNumbers array to the console</li></ul> </li></ol>"
        },
...
app.controller('jsEventDelegationController', function() {
    ...
    this.features = {
        ...
        'Feature Set 1 - The lonely event listener': {
            html: "<ol class='left-border'><li>Directly target the <code>button</code>s within <code>#list</code> with jQuery and add an <code>.on()</code> handler to:</li><li><code>console.log()</code> the button's text node:<ul><li>example - <code>$( \"#list button\" )</code> </li><li>The console should read:<ul><li>if the first button is clicked: \"Button #1\" </li><li>if the second button is clicked: \"Button #2\" etc.</li></ul></li></ul></li></ol>"
        },
        'Feature Set 2 - Bring a friend': {
            html: "<ol class='left-border'><li>After the previous feature set's code, create another button using jQuery DOM Creation:<ul><li>example - <code>\"&lt;li&gt;&lt;button style='margin-top: 10px'&gt;Delegated Button#5 Handler&lt;/button&gt;&lt;/li&gt;\"</code></li><li>Add the button to the <code>#list</code> with <code>.append()</code></li></ul></li><li>Add comments to the top of your <code>script.js</code> file, answering the following questions:<ol><li>What do you think is going to happen when you click on the last button?</li><ul><li>Why do you think this is going to happen?</li></ul><li>What happened when you clicked on it?</li><ul><li>Why did this happen?</li></ul></ol></li></ol>"
        },
        ...
        'Additional Challenge': {
            html: "<ol class='left-border'><li>Create a new button that has a unique attribute which, when clicked, opens a new tab and navigates to <a href=\"google.com\">google.com</a>.</li><li>Create a jQuery delegated <code>.on()</code> handler containing a conditional that only fires if the clicked element contains that unique HTML attribute. </li></ol>"
        },
        "Don't forget to test all of your work": {
            html: "<ul class='left-border'><li>Use the console to verify all of your work</li><li>If you are having trouble with any of the feature sets, ask for help</li></ul>"
        }
